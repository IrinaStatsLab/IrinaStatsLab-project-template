...
New to the lab? Before reading the full principles below, here is the minimum viable checklist for reproducible, shareable work. These correspond to the 🔴 **Core** principles — the ones where violations cause immediate, concrete problems.

- [ ] **Open the `.Rproj` file in RStudio** — never navigate directly to a script. This sets the working directory to the project root automatically.
- [ ] **Use relative paths only** — write `"data/raw/file.csv"`, never `/Users/yourname/...`. No `setwd()` anywhere in the project.
- [ ] **Never write to `data/raw/`** — treat all raw data as read-only.
- [ ] **Never click Export in the RStudio Plots pane** — all figures and tables must be generated by code; clicking Export breaks the code-to-output trail silently.
- [ ] **Put `set.seed()`** at the top of any script that uses randomness.
- [ ] **Disable workspace saving** — one-time RStudio setting: Tools → Global Options → General → set "Save workspace to .RData on exit" to **Never**; uncheck "Restore .RData into workspace at startup".
- [ ] **Commit to version control regularly** — with meaningful messages, not as a single end-of-project dump.

The full Principles section below explains the reasoning behind each item and what to build toward as projects grow in size, duration, and complexity.
...
> ⏳ *Learning curve:* Like functions/scripts separation, this becomes most visible when something needs to change — a recoding decision, a model specification — and you realize the logic has been copy-pasted into four different scripts. Fixing it once is easy; fixing it in four places consistently is error-prone.
...
> ⏳ *Learning curve:* The first time you wait 20 minutes for a simulation or model to rerun just to tweak a figure label, this principle becomes self-evident. For fast pipelines it adds little; for anything computationally expensive it is essential.
